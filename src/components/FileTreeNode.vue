    ...
    >
      <div class="w-4 h-4 flex items-center justify-center shrink-0">
        <template v-if="node.isDir">
          <ChevronDown v-if="expanded" class="w-3.5 h-3.5 text-on-surface-variant group-hover:text-on-surface transition-colors" />
      ...
      <span class="text-body-sm font-medium flex-1 truncate" :class="{
        'text-on-surface': node.isActive || (node.isDir && expanded),
        'text-on-surface-variant': !node.isActive && (!node.isDir || !expanded)
      }">{{ node.name }}</span>
      
      <!-- Progress / Size -->
      <div class="text-[10px] text-on-surface-variant/70 tabular-nums shrink-0 flex items-center gap-2">
        <span v-if="node.isActive && !node.isDir" class="text-primary font-bold">{{ node.progress }}%</span>
        <span>{{ formatSize(node.size) }}</span>
      </div>
    </div>
    
    <div v-if="node.isDir && expanded && Object.keys(node.children).length > 0" class="pl-5 ml-1 border-l border-white/5 flex flex-col gap-0.5 mt-0.5">
